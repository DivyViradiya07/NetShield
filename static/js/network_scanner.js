...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchOpenPorts() {
        try {
            ...
            appendLog(`[!] Error fetching open ports: ${error.message}`);
        }
    }

    // Function to update the open ports table
    function updateOpenPortsTable(ports) {
        openPortsTableBody.innerHTML = ''; // Clear existing rows
        if (ports.length === 0) {
        ...
        ports.forEach((p, index) => {
            ...
            row.classList.add('hover:bg-gray-700'); // Add hover effect
            row.innerHTML = `
                <td class="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-300">${index + 1}</td>
                <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-300">${p.port}</td>
                <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-300">${p.protocol}</td>
                <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-300">${p.service}</td>
                <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-300">${p.version}</td>
                <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-300">${p.process_name}</td>
            `;
